...
The research team realized that there has been no research on integrating cybersecurity awareness training (CAT) with other components of a defense-in-depth strategy in a corporation. For example, there is no robust mechanism to integrate CAT with Risk management, threat modeling and even with cybersecurity operation centers.

Therefore, this paper proposes the concept of a cyber awareness map which is a combination of a concept map (built from learners' perceptions about cybersecurity attack vectors), a threat map (based on the National Vulnerability Database), and an education feedback map (built from the team's quiz assessments of learners' knowledge about existing attack vectors).

The project is a multi-team project separated into two main modules, one for each team in each class. DA ( the Data Analytic) module will be worked on within the scope of CSC522 class and be worked on by all team members. DE ( the cyber awareness DElivery module) will be worked on within the scope of CSC591 class with Tam Nguyen as the 1-man team. The following sections will mostly mention details relating to the CSC591 part of the project.

\subsection{Research Questions}
\begin{enumerate}
\item What is the perception of financial/banking employees towards cyber attack vectors?\\
(Financial/banking sector is the second most vulnerable to cybersecurity.)
\item What is the baseline of their cyber malicious item spotting skill?\\
(The habit of paying attention to visual cues is more important than the detailed knowledge of past exploits. This habit may help financial employees avoid future attacks.)
\item Can we improve the malicious item spotting skill via light weight but frequent cyber awareness training?\\
(Threat landscape is constantly changing. Light-weight assessments like Qualtric-based quizzes are faster to make, cheap and can be effective.) 
\item How does cyber awareness training change their perceptions about cyber security attack vectors over time?\\
(We will look for evidence of perception changes to be incorporated into DA component and the DA-DE feedback loop.)
\end{enumerate}

\subsection{Research Goals}
\begin{enumerate}
\item Establish a perception map of at least 10 cyber attack vectors (web, mobile, windows, iot..) as perceived by 500 financial/banking employees in the US
\item Establish a base-line score reflecting the employees skill of spotting potential malicious area(s) within graphical interface screen-shots.
\item Design, test, and measure a new graphic-based yet light-weight cyber awareness training and assessment system.
\item Showing whether or not the light-weight cyber awareness training system can change the employees' perceptions toward cyber attack vectors over time.
\item Derive several interesting insights from the cyber awareness map.
\end{enumerate}


\section{Schedules and Deliverables}
...
The survey will be Qualtric-based, has around 15 carefully crafted questions, 10 of which will be quiz questions. Survey will be peer-reviewed by security professionals from outside of the team to ensure questions make sense and are accurate. Survey will be distributed to Amazon MTurk platform. 500 entries will be expected.
\subsection{Analyzing base-line results & Design quizzes - end by 27OCT}
Survey results will be analyzed and be used to plot the initial perception map. Based on this map and inputs from DA team, the perception map can be upgraded into the cyber threat awareness map. 
\subsection{Run quiz campaigns - end by 30NOV}
Light-weight educational quizzes (maximum of 3 questions) will be distributed to the MTurkers who participated in the base-line campaign. 100 entries will be the limit for each round. At least 3 rounds will be executed.
\subsection{Evaluating, integrating data into perception map - end by 30NOV}
Statistics gathered from survey rounds will be gradually analyzed, logged and be reflected in the map.
\subsection{Second base-line survey campaign - end by 05DEC}
The same survey structure with the same question bank that were used for the initial base-line campaign will then be used again for this phase.  The survey will be made available just for the participants in the initial base-line campaign.
